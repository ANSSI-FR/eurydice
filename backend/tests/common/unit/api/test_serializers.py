...
class TestAssociationTokenSerializer:
    ...
    @pytest.mark.parametrize(
        ...
        [
            bytes2words.encode(
                b"\xddy\xba\xe1\x85\x8c\x0c6\xdai\xacw\x0clq\xe4\xb0\xf4\xc1\x0cs\xacf\t\rO\x07\xa0e3P\x80\xd4\xee"
            ),
            "abusif potatoe",
            "1ENDRAIS epamprerions SOURIAIT HEAUMIERS demunissant devissez liftier",
            "CENDRAIS 1pamprerions SOURIAIT HEAUMIERS demunissant devissez liftier",
            "🏊🏻ENDRAIS 1pamprerions SOURIAIT HEAUMIERS demunissant devissez liftier",
            "%ENDRAIS 1pamprerions SOURIAIT HEAUMIERS demunissant devissez liftier",
        ],
    )
    ...
    def test_to_internal_value_error_invalid_token_signature(self, settings: conf.Settings, faker: Faker):
    ...
    def test_to_internal_value_error_expired_token(self, faker: Faker):
        ...
        with pytest.raises(drf_serializers.ValidationError, match="The association token has expired."):
            serializers.AssociationTokenSerializer().to_internal_value({"token": bytes2words.encode(token.to_bytes())})
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            b"la variation du PIB dans le monde suit tres exactement la variation de la quantite de petrole produite",
        ),
    ],
)
class TestBytesAsHexadecimalField:
    def test_to_internal_value_success(self, representation: str, value: bytes):
        assert serializers.BytesAsHexadecimalField().to_internal_value(representation) == value

    def test_to_representation_success(self, representation: str, value: bytes):
        assert serializers.BytesAsHexadecimalField().to_representation(value) == representation
