...
class TestAssociationToken:
    ...
    def test_decode_error_expired_token(
        self, token: association.AssociationToken, faker: Faker
    ) -> None:
        token.expires_at = faker.date_time_this_decade(
    ...
    def test_set_expires_at_None_success(  # noqa: N802
        ...
        assert token.expires_at == timezone.now().replace(
            microsecond=0
        ) + datetime.timedelta(seconds=settings.USER_ASSOCIATION_TOKEN_EXPIRES_AFTER)

    @freezegun.freeze_time("2021-05-19 03:21:34.123456")
    def test_set_expires_at_tz_aware_datetime_success(
        self, token: association.AssociationToken
    ) -> None:
        token._expires_at = None
