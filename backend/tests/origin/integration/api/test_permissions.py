...
class TestIsTransferableOwner:
    def test_has_object_permission_authorized(self):
        obj = factory.OutgoingTransferableFactory()
        request = mock.Mock()
        request.user.id = obj.user_profile.user.id

        assert permissions.IsTransferableOwner.has_object_permission(self=None, request=request, view=None, obj=obj)
    ...
    def test_has_object_permission_unauthorized(self, faker: Faker):
        ...
        with pytest.raises(http.Http404):
            permissions.IsTransferableOwner.has_object_permission(self=None, request=request, view=None, obj=obj)
