...
class TestUser:
    def test_post_association_token_success_no_user_profile(self, settings: conf.Settings, api_client: test.APIClient):
        ...
        assert auth.get_user_model().objects.filter(id=user.id, user_profile__isnull=True).exists()
        ...
        assert auth.get_user_model().objects.get() == user
        assert models.UserProfile.objects.get(user=user.id, associated_user_profile_id=token.user_profile_id)
    ...
    def test_post_association_token_success_existing_user_profile(
        ...
        factory.UserProfileFactory(user=None, associated_user_profile_id=token.user_profile_id)
    ...
    def test_post_association_token_error_not_authenticated(self, api_client: test.APIClient):
        url = django.urls.reverse("user-association")
    ...
    def test_post_association_token_error_non_readable_token(self, api_client: test.APIClient):
        user = factory.UserFactory()
        assert not models.UserProfile.objects.exists()
    ...
    def test_post_association_token_error_expired_token(self, faker: Faker, api_client: test.APIClient):
        token = common_factory.AssociationTokenFactory(
            expires_at=faker.date_time_this_decade(tzinfo=timezone.get_current_timezone())
        )
    ...
    def test_post_association_token_error_already_associated(self, settings: conf.Settings, api_client: test.APIClient):
        ...
        assert str(response.data["detail"]) == "The user is already associated with a user from the origin side."
        assert models.UserProfile.objects.get() == user_profile
